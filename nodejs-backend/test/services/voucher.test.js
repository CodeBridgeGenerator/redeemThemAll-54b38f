...
describe("voucher service", async () => {
  ...
  describe("#create", () => {
    const options = {"name":"new value","total":true};
...
assert.strictEqual(voucherCreated.total, options.total);
assert.strictEqual(voucherCreated.total, options.total);
    });
  });
  ...
  describe("#update", () => {
    const options = {"name":"updated value","total":false};
...
assert.strictEqual(voucherUpdated.total, options.total);
assert.strictEqual(voucherUpdated.total, options.total);
    });
  });
