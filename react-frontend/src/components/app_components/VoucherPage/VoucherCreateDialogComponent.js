...
const VoucherCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,total: _entity?.total || false,
...
<div className="col-12 md:col-6 field flex">
            <span className="align-items-center">
                ...
                <Checkbox id="total" className="ml-3" checked={_entity?.total} onChange={(e) => setValByKey("total", e.checked)}  />
            </span>
            <small className="p-error">
